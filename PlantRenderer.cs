...
{
    [Tooltip("The starting radius of the plant's main trunk or stem.")]
    public float baseRadius;
    [Tooltip("The multiplier applied to the radius at each branching point (e.g., 0.707).")]
    public float radiusDecay;
    [Tooltip("The default length of a single growth segment.")]
    public float defaultStep;
    [Tooltip("The default angle used when branching or turning, if not otherwise specified.")]
    public float defaultAngleDeg;
    
    [Tooltip("Multiplier for stem elongation over iterations.")]
    public float elongationRatio;
    [Tooltip("Multiplier for stem thickening over iterations (Pipe model).")]
    public float fatteningRatio;
    
    [Tooltip("Primary divergence angle (e.g., Golden Angle 137.5 for phyllotaxis).")]
    public float divergenceAngle1;
    [Tooltip("Secondary divergence angle for alternate branching patterns.")]
    public float divergenceAngle2;
    
    [Tooltip("Primary pitch/branching angle from the parent stem.")]
    public float branchAngle1;
    [Tooltip("Secondary pitch/branching angle for specific lateral shoots or floral stems.")]
    public float branchAngle2;
    
    [Tooltip("Specific angle used for downward bending (e.g., pedicel drooping).")]
    public float pitchAngle;
    [Tooltip("Specific angle used for rolling around the local axis.")]
    public float rollAngle;
    
    [Tooltip("Primary internode length scaling factor.")]
    public float internodeLen1;
    [Tooltip("Secondary internode length scaling factor.")]
    public float internodeLen2;
    
    [Tooltip("Scaling multiplier for leaf geometry.")]
    public float leafSize;
    [Tooltip("Scaling multiplier for flower geometry.")]
    public float flowerSize;
    [Tooltip("Scaling multiplier for bud geometry.")]
    public float budSize;
    [Tooltip("Scaling multiplier for fruit/seed pod geometry.")]
    public float fruitSize;
    
    [Tooltip("Primary probability for stochastic rule selection (0.0 to 1.0).")]
    public float probPrimary;
    [Tooltip("Secondary probability for stochastic rule selection (0.0 to 1.0).")]
    public float probSecondary;
}
...
private PlantParams plantSettings = new()
{
    ...
    elongationRatio = 1.109f,
    fatteningRatio = 1.732f,
    divergenceAngle1 = 94.74f,
    divergenceAngle2 = 132.63f,
    branchAngle1 = 18.95f,
    branchAngle2 = 22.0f,
    pitchAngle = 9.0f,
    rollAngle = 137.5f,
    ...
    void BuildLeafMesh(List<PlantNode> nodes, LeafShape shape, int lodDetail, bool fullyGrown, FilterMode filter, Mesh targetMesh)
    {
        ...
        foreach (var node in nodes)
        {
            ...
            switch (shape)
            {
                ...
                case LeafShape.Lyrate: EmitLyrateLeaf(pos, fwd, right, faceN, size); break;
            }
        }
    ...
    void BuildFlowerMesh(List<PlantNode> nodes, FlowerShape shape, bool fullyGrown, FilterMode filter, Mesh targetMesh)
    {
        ...
        foreach (var node in nodes)
        {
            ...
            switch (shape)
            {
                ...
                case FlowerShape.FiveRayHead: EmitFiveRayFlowerHead(center, axis, right, up2, size); break;
            }
        }
